...
import DeleteTask from '@/Pages/Task/Delete.vue';
...
const parentOptions = computed(() => {
    ...
    props.tasks
        .filter((t) =>
            Number(t.project_id) === Number(projectId.value)
            && Number(t.id) !== Number(taskId.value)
            && Number(t.depth ?? 0) < 2,
        )
        .forEach((t) => {
...
function openModal(task) {
    ...
    // 1. Заполняем форму из задачи
    taskId.value = task.id;
    title.value = task.title ?? '';
    projectId.value = task.project_id ?? null;
    parentId.value = task.parent_id ?? null;
    status.value = enumValue(task.status, 'started');
    difficulty.value = enumValue(task.difficulty, 'normal');
    dueAt.value = formatDueAt(task.due_at);
    setMembers(task.workers ?? []);
    error.value = '';
...
async function submit() {
    ...
    try {
        ...
        // 2. Обновляем задачу
        const { data } = await axios.post(route('tasks.update', taskId.value), formData);
    ...
    <Modal :show="showModal" max-width="lg" @close="closeModal">
        <div class="space-y-4 p-4 md:p-6">
            <div class="flex items-center justify-between gap-3">
                <h2 class="title-2">Изменить задачу</h2>
                <button
                    type="button"
